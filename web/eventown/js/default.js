...
function sendTab(){
    ...
    var city_id = $('#select-city-id').val();
    var city_name = $('#select-city').val();
    var people_key=$('#place_people_num').attr('data-values');
    var place_people_num = $('#place_people_num').val();
    if(people_key==0){
        p=0;
    }else if(people_key==6){
        p=parseInt(place_people_num);
    }else{
        p=parseInt(place_people_num.split('-')[1]);
    }
    window.location.href='/newweixin/rfp?city_id='+city_id+'&city_name='+city_name+'&place_people_num='+p;
}
// 禁止焦点
