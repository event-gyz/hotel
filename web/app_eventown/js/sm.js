        ...
        p.updateValue = function () {
            ...
            for (var i = 0; i < p.cols.length; i++) {
                if (!p.cols[i].divider) {
                    ...
                    newIndex.push(p.cols[i].activeIndex)
                }
            }
            ...
            p.index=newIndex;
            if (p.params.onChange) {
                p.params.onChange(p, p.value, p.displayValue,p.index);
            }
            if (p.input && p.input.length > 0) {

                $(p.input).val(p.params.formatValue ? p.params.formatValue(p, p.value, p.displayValue,p.index) : p.value.join(' '));
                $(p.input).attr("data-id",p.index)
                $(p.input).trigger('change');
            }
        };
        ...
        p.initPickerCol = function (colElement, updateItems) {
            ...
            col.updateItems = function (activeIndex, translate, transition, valueCallbacks) {
                ...
                if (valueCallbacks || typeof valueCallbacks === 'undefined') {
                    ...
                    col.Index=selectedItem.attr('data-picker-value');
                    col.displayValue = col.displayValues ? col.displayValues[activeIndex] : col.value;
                    // On change callback
                    if (previousActiveIndex !== activeIndex) {
                        if (col.onChange) {
                            col.onChange(p, col.value, col.displayValue,col.Index);
                        }
                        p.updateValue();
                    }
                }
        ...
        p.columnHTML = function (col, onlyItems) {
            ...
            if (col.divider) {
                ...
                for (var j = 0; j < col.values.length; j++) {
                    columnItemsHTML += '<div class="picker-item" data-picker-value="' + col.values[j] + '" data-value="' + j + '">' + (col.displayValues ? col.displayValues[j] : col.values[j]) + '</div>';
                }
...
+ function($) {
    ...
    var defaults = {
        ...
        onChange: function (picker, values, displayValues,index) {
        ...
        formatValue: function (p, values, displayValues,index) {
