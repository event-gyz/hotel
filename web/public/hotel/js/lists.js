...
var VM = new Vue({
    ...
    mounted: function(){
        ...
        this.$nextTick(function () {
            ...
            $('body').on('click','#searchClear',function(){
                _this.seachWorld = '';
                _this.seachFworld(_this.query_params.city_id,'')
            });
    ...
    methods: {
        ...
        query: function(type,id,name,location){
            if(type == 'metro' || type == 'business'){
                this.query_params.position = '';
                this.query_params.location = location;
            }else if(type == 'train'){
                this.query_params.position = type+'_station_id-'+id;
        ...
        //酒店位置的地铁搜索
        query_con: function(id){
            var _this = this;
            _this.metro_line = id;
            $.ajax({
                url:api_url+'/place/get_metro_stations',
                type:'post',
                dataType: 'json',
                data:{
                    line_id:id
        ...
        queryPosition: function(){
            var _this = this;
            $.ajax({
                url:api_url+'/place/get_position',
                type:'post',
                dataType: 'json',
                data:{city_id:_this.query_params.city_id},
                success: function(responseData){
                    if(!responseData.errorno){
                        _this.position = responseData.data;
                    }else{
                        alert(responseData.msg);
                    }
                    $('#global_login').hide();
        ...
        seachFworld:function(cid,val){
            ...
            var url = api_url+'/hotel/search_by_keywords';
